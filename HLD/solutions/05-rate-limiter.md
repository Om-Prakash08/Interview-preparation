...
## SECTION 4 — Algorithm Deep Dive (MOST IMPORTANT for this question)
...
**Problem**: 
- Stores every single request timestamp
- 100 req/min × 10M users = **1 billion timestamps in Redis** → too much memory
...
### Deep Dive 1: Distributed Rate Limiting (The Hard Part)
...
### Deep Dive 2: Race Condition Prevention
...
### Deep Dive 3: Handling Redis Failure (Fail Open vs Fail Closed)
...
### Deep Dive 4: Multi-level Rate Limiting
...
2. "For algorithm: I'd use **Token Bucket** — allows controlled bursts, easy to tune"
3. "For distribution: **centralized Redis** so all API servers share the same counter"
4. "Atomicity is critical — I'd use a **Lua script** in Redis to prevent race conditions"
5. "On Redis failure: **fail open** — brief unprotected window is better than full outage"
6. "Headers: always return `X-RateLimit-Remaining` and `Retry-After` — good API design"
7. "Multi-tier: support different limits per user tier (free vs paid) via a rules config service"
