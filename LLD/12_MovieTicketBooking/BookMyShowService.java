...
public class BookMyShowService {
    ...
    public static synchronized BookMyShowService getInstance() {
        if (instance == null) instance = new BookMyShowService();
        return instance;
    }

    public void addCinema(Cinema cinema) {
        cinemas.add(cinema);
        for (Show show : cinema.getShows()) shows.put(show.getShowId(), show);
    }
        ...
        for (Cinema cinema : cinemas)
            if (cinema.getCity().equalsIgnoreCase(city))
                for (Show show : cinema.getShows())
                    if (show.getMovie().getTitle().equalsIgnoreCase(movieTitle)) result.add(show);
        return result;
    }

    public Booking bookTickets(String showId, List<String> seatIds, String userName) {
        Show show = shows.get(showId);
        if (show == null) { System.out.println("[Booking Error] Show not found."); return null; }
        ...
        synchronized (show) {
            ...
            for (String seatId : seatIds) {
                ...
                if (seat == null) { System.out.printf("[Booking Failed] Seat %s does not exist!%n", seatId); return null; }
                if (seat.isBooked()) { System.out.printf("[Booking Failed] Seat %s is already booked!%n", seatId); return null; }
                seatsToBook.add(seat);
            }

            for (ShowSeat seat : seatsToBook) seat.reserve();
